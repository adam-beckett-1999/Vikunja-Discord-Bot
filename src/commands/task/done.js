...
export const data = new SlashCommandBuilder()
  .setName('task-done')
  .setDescription('Mark a Vikunja task as done')
  .addStringOption((opt) =>
    opt.setName('project')
  ...
  .addStringOption((opt) =>
    opt.setName('task')
      .setDescription('Task to mark as done')
      .setRequired(true)
      .setAutocomplete(true)
  );
...
export async function execute(interaction) {
  ...
  if (task.done === true) {
    await interaction.editReply({
      embeds: [buildErrorEmbed('Task is already marked as done.')],
    });
    return;
  }
  ...
  try {
    ...
    const res = await updateTask(task.id, { done: true });
